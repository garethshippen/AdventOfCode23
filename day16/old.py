...
class Node():
    ...
    def valid_neighbour(cls, path, coord):
        def sub_coord(first, second):
            return(second[0] - first[0], second[1] - first[1])
        if len(path) < 3:
            return True
        exhibit = path[-3:]
        exhibit.append(coord)
        moves = []
        for i in range(len(exhibit)-1):
            moves.append(sub_coord(exhibit[i], exhibit[i+1]))
        if len(set(moves)) == 1:
            return False
        else:
            return True
    ...
    def set_h(self): 
        try:
            return abs(Node.x_size - self.coord[0]) + abs(Node.y_size - self.coord[1])
        except:
...
for node in a_star[end].route:
    grida[node] = a_star[node].get_g()
    
for y in range(13):
    for x in range(13):
        print(str(grida[(x,y)]) + " ", end = "")
    print()
